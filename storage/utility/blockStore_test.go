...
func TestBlockStore_Store_LoadBlockStore(t *testing.T) {
	...
	for i, v := range values {
		...
		bs, err := NewBlockStore(v.numBlocks, v.blockSize, versioned.NewUnbufferedKV(make(ekv.Memstore)))
		if err != nil {
...
func TestBlockStore_loadBlock_LoadStorageError(t *testing.T) {
	...
	bs := &BlockStore{kv: versioned.NewUnbufferedKV(make(ekv.Memstore))}
...
func TestBlockStore_load_KvGetError(t *testing.T) {
	...
	testBS := &BlockStore{kv: versioned.NewUnbufferedKV(make(ekv.Memstore))}
	err := testBS.load()
	if err == nil || !strings.Contains(err.Error(), expectedErr) {
