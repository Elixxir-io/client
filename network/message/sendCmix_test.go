...
func (mc *MockSendCMIXComms) SendPutManyMessages(host *connect.Host, messages *mixmessages.GatewaySlots) (*mixmessages.GatewaySlotResponse, error) {
	return &mixmessages.GatewaySlotResponse{
		Accepted: true,
		RoundID:  3,
	}, nil
...
func Test_attemptSendManyCmix(t *testing.T) {
	...
	_, _, err = sendManyCmixHelper(messages, recipients, params.GetDefaultCMIX(),
		m.Instance, m.Session, m.nodeRegistration, m.Rng,
		m.TransmissionID, &MockSendCMIXComms{t: t})
	if err != nil {
		t.Errorf("Failed to sendcmix: %+v", err)
